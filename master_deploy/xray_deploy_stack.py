...
class XrayDeployStack(cdk.Stack):
    def __init__(self, scope: cdk.Construct, id: str, vpc_id, **kwargs) -> None:
        ...
        xray = xray_task.add_container(
            "xray-daemon",
            start_timeout=cdk.Duration.seconds(60),
            stop_timeout=cdk.Duration.seconds(60),
            image=ecs.ContainerImage.from_registry("amazon/aws-xray-daemon"),
            memory_reservation_mib=256,
        )

        xray_port_udp = ecs.PortMapping(container_port=2000, protocol=ecs.Protocol.UDP)
        xray_port_tcp = ecs.PortMapping(container_port=2000, protocol=ecs.Protocol.TCP)
